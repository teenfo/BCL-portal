...
**기획서**: `03-operations.md` ⚠️ **개선 필요**  
**구현 파일**: `src/app/admin/operations/schedule/page.tsx` ✅

**점검 결과**:
- ✅ **세션 목록**: 일간 세션 조회 및 정원 표시
- ⚠️ **드래그 앤 드롭**: 기획서에서 요구한 "주간 캘린더 + D&D" 미구현
  - 현재: 리스트 뷰 + 수동 등록 모달
  - 권장: React Big Calendar 또는 FullCalendar 라이브러리 추후 도입

**기획서 요구사항 vs 현재 구현**:
| 기획 요구사항 | 현재 구현 | 비고 |
|--------------|----------|------|
| 주간 캘린더 뷰 | ❌ | 일간 리스트 뷰로 구현됨 |
| 드래그 앤 드롭 시간 조정 | ❌ | 모달 기반 수동 입력 |
| 코치 중복 배정 방지 | ⚠️ | 클라이언트 체크 없음 (DB RLS로 처리 예정) |
...
### 🟠 중요 (DB 연동 초기 단계)
1. **Schedule 페이지 개선**: 주간 캘린더 라이브러리 도입 검토
2. **Dashboard RPC**: 복잡한 KPI 계산은 DB 함수로 이관
3. **Real-time 구독**: Checkins 페이지 실시간 업데이트

### 🟡 권장 (고도화 단계)
1. **Image Upload API**: Supabase Storage 연동 완성
2. **Banner 관리 기능**: Content 페이지 배너 탭 완성
3. **Advanced Filtering**: 각 목록 페이지에 날짜 범위 필터 추가
...
| 평가 항목 | 점수 | 비고 |
|----------|------|------|
| 기획-구현 일치도 | 95/100 | Schedule 페이지 드래그 앤 드롭 미구현 |
