...
1. **정규 수업 레이싱 모드 (Racing Mode - 본 레이스 시스템)**
   ...
   - **참여 포맷 (개인전 및 팀전)**: 모든 참가자가 각자 달리는 개인전(Individual) 뿐만 아니라, **팀전(Team Competition) 모드**를 지원합니다. 특정 레인들을 A팀, B팀으로 묶어 여러 기기의 전진 거리/와트를 합산 계산하거나, 릴레이(교대) 방식으로 대형 스크린에 팀별 아바타/보트가 전진하는 다이내믹한 기능을 제공합니다.
