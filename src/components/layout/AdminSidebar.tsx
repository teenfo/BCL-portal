...
const menuGroups: MenuGroup[] = [
    ...
    },
    {
        label: 'QUICK LINKS',
        items: [
            { name: '사용자 앱', href: '/apps/dashboard', icon: <IconUserApp /> },
            { name: '코치 앱', href: '/coach/dashboard', icon: <IconCoachApp /> },
            { name: '키오스크', href: '/kiosk', icon: <IconKiosk /> },
        ],
    },
];
        ...
        >
            ...
            <nav className="admin-sidebar__nav custom-scrollbar">
                ...
                {menuGroups.map((group, groupIndex) => {
                    const isQuickLinks = group.label === 'QUICK LINKS';
                    return (
                        <div
                            key={group.label}
                            className="admin-sidebar__group"
                            style={isQuickLinks && !collapsed ? {
                                margin: '4px 12px 0',
                                padding: '10px',
                                borderRadius: '14px',
                        ...
                        >
                            ...
                            {!collapsed ? (
                                <div
                                    ...
                                    style={isQuickLinks ? { color: 'rgba(255,107,0,0.65)', display: 'flex', alignItems: 'center', gap: '5px' } : undefined}
                                >
                                    {isQuickLinks && (
                                ...
                                {group.items.map((item) => {
                                    ...
                                    return (
                                        <Link
                                            key={item.href}
                                            href={item.href}
                                            target={isQuickLinks ? '_blank' : undefined}
                                            rel={isQuickLinks ? 'noopener noreferrer' : undefined}
                                            className={`admin-sidebar__item ${isActive ? 'admin-sidebar__item--active' : ''}`}
                                            title={collapsed ? item.name : undefined}
                                        >
                                            <span
                                                className="admin-sidebar__icon"
                                                style={isQuickLinks ? { color: 'var(--primary)' } : undefined}
                                            >
                                                {item.icon}
                                            </span>
                                            {!collapsed && (
                                            ...
                                            {!collapsed && isQuickLinks && (
                                                <span style={{ marginLeft: 'auto', opacity: 0.35, display: 'flex', alignItems: 'center' }}>
