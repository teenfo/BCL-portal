...
import { IconSmartphone, IconBuilding, IconMonitor } from '@/components/icons/AdminIcons';
            ...
            <div className="p-10 max-w-[1400px] mx-auto">
                {loading ? (
                    ...
                    <div className="space-y-12">
                        <section>
                            <h3 className="text-xl font-black text-white uppercase mb-6 inline-flex items-center gap-2"><IconSmartphone size={20} /> QR 코드 관리</h3>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {facilities.map((f) => (
                                    <div key={f.id} className="glass-card p-6 rounded-2xl">
                                        <div className="flex items-center gap-3 mb-4">
                                            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg bg-white/[0.03] border border-white/5"><IconBuilding size={20} /></div>
                                            <div><h4 className="text-sm font-black text-white">{f.name}</h4><p className="text-[9px] text-[var(--text-muted)]">{f.address || '-'}</p></div>
                                        </div>
                                        <button className="w-full py-2.5 rounded-xl text-[9px] font-black uppercase transition-all" style={{ background: 'rgba(255,107,0,0.1)', border: '1px solid rgba(255,107,0,0.2)', color: 'var(--primary)' }}>QR 코드 생성</button>
                                    </div>
                                ))}
                            </div>
                        </section>
                        ...
                        <section>
                            <h3 className="text-xl font-black text-white uppercase mb-6 inline-flex items-center gap-2"><IconMonitor size={20} /> 키오스크 원격 제어</h3>
