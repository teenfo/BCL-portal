...
### 9) Circuit Station Rotation HUD (`/class/rotation-hud`) 🆕 (Priority 25)
- **구현 파일**: `src/app/class/rotation-hud/page.tsx`
- **Device Type**: Desktop (TV/대형 모니터, 16:9 가로 전용)
- **목적**: 서킷 스테이션 팀 로테이션 현장 전광판 — 스테이션별 운동 구성 및 실시간 배정 조원 카드 렌더링.
- **주요 기능**:
  - 6분할 스테이션 매트릭스 그리드 뷰 및 카테고리별 비주얼 글로우 적용.
  - 5분 카운트다운 인터벌 HUD 타이머 (마지막 10초 붉은색 경고 점멸).
  - 5분 라운드 종료 시 ROTATE 플래시 오버레이 및 경고 사이렌 사운드 지원.
  - 로테이션 전환 시 팀원 카드가 시계 방향으로 부드럽게 미끄러지며 정렬되는 CSS 스왑 슬라이딩 애니메이션.
  - Supabase Realtime 리모컨 신호(시작/정지/강제 순환) 실시간 동기화 수신.
