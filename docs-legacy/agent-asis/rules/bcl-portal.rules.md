...
- 개발된 기능은 **`/audit` 워크플로우를 통해 3자 검토**를 거쳐야 한다.
- `/audit`는 **개발에 참여하지 않은 다른 모델**이 수행한다:
  - 개발 모델의 셀프 체크(빌드, 타입)와는 별개의 독립 워크플로우
  - 기획서와 코드를 처음부터 읽고, 정합성/보안/UI 표준을 전체 스캔
- **실행 시점**:
  - Priority 단위 개발 완료 후 (권장)
  - 보안/결제 관련 변경 후 (필수)
- 발견된 이슈는 심각도에 따라 **즉시 수정** 또는 **Blueprint Known Issues에 등록**한다.
