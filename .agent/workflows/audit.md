...
> 📌 **`/develop`와의 관계**:
> - `/develop` Step 4: 개발 모델이 하는 **셀프 체크** (빌드, 타입, 기본 회귀)
> - `/audit`: 다른 모델이 하는 **3자 검토** (전체 스캔, 기획-코드 정합성)
> - 두 단계는 독립적이며, `/develop` 완료 + 커밋 후 `/audit`를 별도 실행합니다.
...
```
1. 기획서 (Source of Truth)
   └── .docs/planning/*.md 또는 .docs/archive/planning/*.md
...
### 1️⃣ 감사 대상 확인
**수행**: 감사할 대상(Priority, 기능, 라우트)을 확인합니다.

// turbo
1. 블루프린트에서 최근 완료된 Priority 확인:
   ```bash
...
2. 해당 기획서 로드:
   ```bash
   ls .docs/planning/ .docs/archive/planning/
   ```
...
> 💡 **핵심**: 개발 히스토리를 모르는 상태에서, 기획서와 코드를 처음부터 읽고 비교합니다.

#### A. 기획-코드 정합성 (🏛️ Architect 관점)
- [ ] 기획서의 Phase별 요구사항이 **실제로** 구현되어 있는가?
- [ ] Sitemap의 라우트 구조와 실제 파일 구조가 일치하는가?
- [ ] 기획서에 명시된 Edge Case/예외 상황이 코드에 반영되었는가?
...
## 4. 조치 요약
- 🔴 즉시 수정: {N}건 (모두 해결)
- 🟡 등록 필요: {N}건 → `/plan-to-blueprint` 실행 시 정식 Priority로 등록
- 🟢 권장: {N}건
```
...
감사에서 발견한 이슈는 심각도에 따라 처리합니다:

| 심각도 | 처리 방법 |
|:-------|:---------|
| 🔴 **즉시 수정** | 감사 세션 내에서 직접 코드 수정, `fix(audit):` 커밋 |
| 🟡 **등록 필요** | 보고서에 기록 → `/plan-to-blueprint` 실행 시 **정식 Priority로 등록** |
| 🟢 **권장 개선** | 보고서에 기록 (다음 개발 시 참고) |
