...
**작업**:
1. Stitch 디자인을 참조하여 React 컴포넌트 구현
2. `ui-gen` 스킬을 사용하여 Glassmorphism 스타일 적용
3. 반응형 디자인 고려 (Stitch는 고정 크기지만 실제는 반응형)

**참조 도구**:
- `mcp_StitchMCP_get_screen()` - 디자인 확인
- `.agent/skills/ui-gen/SKILL.md` - Glassmorphism 가이드
- 기존 컴포넌트 - 재사용 가능한 컴포넌트
...
### 필수 규칙
1. **Sitemap 우선**: 반드시 Sitemap 갱신 후 디자인 생성
2. **기존 프롬프트 참조**: 일관성을 위해 동일 영역의 기존 프롬프트 참조
3. **프롬프트 저장**: 생성에 사용한 프롬프트는 반드시 저장
4. **Screen ID 매핑**: Sitemap에 Screen ID 기록 필수
...
## 🔗 관련 문서
- `.antigravity/STITCH_INTEGRATION.md` - Stitch MCP 통합 가이드
