...
## 🤖 Gemini 전용 품질 보완 프로세스 (필수)

Gemini 모델은 초대형 컨텍스트를 활용할 수 있지만, 인라인 스타일이나 하드코딩된 패턴을 새로 생성하는 경향이 있습니다. 이를 방지하기 위해 다음 **3단계 프로세스**를 반드시 준수해야 합니다.

### Step 0: 컨텍스트 최대 로드 (구현 전)
구현 전 다음 정보들을 한 번에 로드하여 "패턴 인식" 능력을 극대화합니다.
1. **유사 페이지**: 동일 영역(apps/admin)의 기존 페이지 코드 **전체** (패턴 참조용)
2. **글로벌 CSS**: `src/app/globals.css` (글로벌 클래스 파악)
3. **타입 정의**: `src/types/` 관련 타입 및 공통 컴포넌트 목록
4. **디자인 가이드**: 이 문서(`ui-gen/SKILL.md`) 전체

### Step 1: 패턴 복사 우선 원칙 (구현 중)
- ❌ **창조 금지**: 새로운 스타일 패턴이나 구조를 직접 설계하지 않습니다.
- ✅ **복사 후 수정**: 기존 페이지에서 가장 성공적인 UI 패턴을 찾아 복사한 뒤 데이터와 라벨만 수정합니다.
- ✅ **글로벌 클래스**: `.admin-filter-btn`, `.admin-action-btn` 등을 최우선으로 사용합니다.

### Step 2: 자체 검증 체크리스트 (구현 후)
작업 완료 보고 전 다음 항목을 1:1로 대조하여 스스로 수정합니다.
- [ ] **인라인 스타일**이 있는가? → 있으면 글로벌 클래스로 교체
- [ ] **TypeScript `any`**가 있는가? → 있으면 정확한 타입으로 교체
- [ ] **하드코딩 색상**(`rgba`, `#...`)이 있는가? → 있으면 CSS 변수로 교체
- [ ] **재사용 가능한 컴포넌트**가 있는데 새로 만들었는가? → 있으면 import로 교체
- [ ] **console.log**가 남아있는가? → 제거
